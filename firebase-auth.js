    ...
    postsContainer.innerHTML = snapshot.docs.map((doc) => {
      ...
      return `<article class="community-post">
        <div class="post-user">
          ...
          <div><h3>${name}</h3><span>${date}</span></div>
        </div>
        <div class="post-content">${content}</div>
      </article>`;
    }).join("");
  }, (error) => {
    postsContainer.innerHTML = `<div class="card"><h3>Firestore error</h3><p>${error.message}</p></div>`;
  });
}
...
window.createCommunityPost = async function(e) {
  ...
  if (!user) {
    status.textContent = "Your login session is still loading. Please wait a moment.";
